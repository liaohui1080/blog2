<template>
  <div>
    <!--      大屏幕显示头部-->
    <div class="hidden sm:hidden md:block home-header">
      ...
      <h3 style="font-size: 14px; color: #eaecef; text-shadow: black 0.1em 0.1em 0.1em">300ge</h3>
    </div>
    <!--      小屏幕显示头部-->
    ...
    <div class="md:mx-auto md:w-[1200px] px-5 ">
      ...
      <div class="md:grid md:grid-cols-4 gap-8">
        <!--             显示所有的 lib库 如果order==0 就在首页不显示了,比如说梅花易数 -->
        <div v-for="(item, key) in libs" v-if="item.frontmatter.order==0?false:true">
...
export default {
  ...
  methods: {
    ...
    pagesObj(item) {
      ...
      pages.map(item2 => {
        //使用decodeURI解码,如果路径里有中文的话需要用这个解码
        if (item === decodeURI(item2.path)) {
